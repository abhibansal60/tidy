...
Repo `/home/abhi/code/jev`, about to become public. Save quota: do not print files back, do not run evals or any `--execute`, do not read `.tidy/`, `data/`, `secrets/`, `.env`, `token*.json`. Read only `git ls-files` content. Run the tests once at the start and once at the end: `.venv/bin/python -m unittest discover -s tests -q`.
